...
Allows select and =org-capture= web page parts & links important for you, converts and stores them in the =org-mode= markup. Effectively turing everything into a movie about the Emacs hacker rogue that hoards everything he likes into his web archive stash.
...
- [[#installation][Installation]]
  ...
  - [[#bookmarklets][Bookmarklets]]
    - [[#firefox][Firefox]]
      - [[#old-versions-of-firefox][Old versions of Firefox]]
...
Be sure =org-protocol= is loaded, and Emacs is ready to recieve connections:
#+BEGIN_SRC elisp
(server-start)
...
You need a suitable =org-capture= template. The default selection key is =w=, it must match the script which would send the request.

#+BEGIN_SRC elisp
("w" "Web site"
  entry (file+olp "~/org/inbox.org" "Subheading" "Web")
  "* %a :website:\n:PROPERTIES:\n:ADDED: %u\nNOTE: %?\n:END:\n%:initial")
#+END_SRC

*Note:* Using =%:initial= instead o =%i= seems to handle multi-line content better.
...
:PROPERTIES:
:ADDED: [2020-01-14 Tue]
:NOTE: Captures webpages and links from web, based on prior code and ideas from org-annotation-helper.el and org-browser-url.el.
:END:
#+END_SRC
...
On some old versions of Firefox, it may be necessary to add the following setting. You may skip this step and come back to it if you get an error saying that Firefox doesn't know how to handle =org-protocol= links.
...
For KDE Plasma 5 - first see if [[https://bugs.kde.org/show_bug.cgi?id=401604][KDE5 feature request]] resolved - if so - just use =System Settings -> Applications=, and send report/update to us.
