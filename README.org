...
Check that the key you want to use is not binded to something:

#+BEGIN_SRC js
:bind <key>
#+END_SRC

For exmaple: =:bind <m>=, or =:bind <F2>=.

If output is =:# <key> == - key is not binded. 
If key has binded function, but you want to use it anyway - =:unbind <key>= before proceeding.

Bind script to the key:
#+BEGIN_SRC js
